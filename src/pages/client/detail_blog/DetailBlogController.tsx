...
const DetailBlogController = () => {
  ...
  const handleLikePost = async ()=>{
    try {
      if(Object.keys(user).length>0){
        let data:any = await likePost({_id:post._id,user_id:user.data[0]._id})
  ...
  return (
    ...
    {!loading&& <DetailBlogView handleLikePost={handleLikePost} likes={likes} user={Object.keys(user).length>0&& user} post={post&&post} />}
    </>
  )
}
